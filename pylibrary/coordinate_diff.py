...
#計算結果を統合して、coordinate_dfとする(functionごとの重要度を既に掛けたもの)
#入力は全要素を行列とした影響関係のマトリクス(sample_df)と背反関係のマトリクス(coordinate_df)
#coordinate_dfの要素名は、sample_dfと同じにする
#coordintae_dfを下地として、計算結果を入力する

def coordinateDiff(sample_df, coordinate_df):
    copy_df = coordinate_df.copy()

    dp_columns_all = list(sample_df.columns)
    dp_index_all = list(sample_df.index)
    dp_columns = list(coordinate_df.columns)
    dp_index = list(coordinate_df.index)
    ...
    for df_index in dp_index:
        for df_col in dp_columns:
            #対角行列の属性に0を挿入
            if df_col == df_index :
                coordinate_df.loc[df_index, df_col] = 0
            #対角以外
            else :
                ...
                if sample_df.loc["説明層", df_col] == 0 and sample_df.loc[df_index, "説明層"] != 0 and sample_df.loc[df_index, "機能層"] > sample_df.loc["機能層", df_col] and (df_index_label in sample_df[sample_df["機能層"] == element_func_y]["説明層"].values) == True:
                    ...
                    for i in name_list:
                        coordinate_df.loc[df_index,df_col] =  coordinate_df.loc[df_index,df_col] + middle_df.loc[i,df_index]*copy_df.loc[df_col,i]
                ...
                #index方向に見て説明層が0の時②
                    ...
                    for i in range(element_func_y - element_func_x):
                        ...
                        if (df_col_label in sample_df[sample_df["機能層"] == i + element_func_x + 1]["説明層"].values) == True:
                            ...
                            diff_func = diff_func + 1
                    for i in name_list:
                        coordinate_df.loc[df_index,df_col] = coordinate_df.loc[df_index,df_col] + middle_df.loc[i,df_col]*copy_df.loc[df_index,i]
                ...
                #index,column方向どちらも説明層が0ではない時③           
                    ...
                    if max_element <= element_func_x and max_element <= element_func_y :
                        ...
                        if element_func_x == element_func_y :
                            coordinate_df.loc[df_index,df_col] = middle_df.loc[df_index,df_col]
                        else :
                            if now_label == min_element_func :
                                if element_func_x > element_func_y:
                                    ...
                                    for i in range(element_func_x - element_func_y):
                                        ...
                                        if (df_index_label in sample_df[sample_df["機能層"] == i + element_func_y + 1]["説明層"].values) == True :
                                            ...
                                            diff_func = diff_func + 1
                                    coordinate_df.loc[df_index,df_col] = middle_df.loc[df_index]
                                else :
                                    ...
                                    for i in range(element_func_y - element_func_x):
                                        ...
                                        if (df_col_label in sample_df[sample_df["機能層"] == i + element_func_x + 1]["説明層"].values) == True:
                                            ...
                                            diff_func = diff_func + 1
                                    coordinate_df.loc[df_index,df_col] = middle_df.loc[df_col]
                            else :
                                #機能層の差分回す
                                ...
                                for i in range(element_func_y - now_label):
                                    ...
                                    if (df_col_label in sample_df[sample_df["機能層"] == i + now_label + 1]["説明層"].values) == True:
                                        ...
                                        diff_func_y = diff_func_y + 1
                                coordinate_df.loc[df_index,df_col] = 2*middle_df.loc[df_index,df_col]
                    else :
                        coordinate_df.loc[df_index,df_col] = 0

                else:
                    coordinate_df.loc[df_index,df_col] = copy_df.loc[df_index,df_col]

    #正規化するためのリストに数値を加える、無駄に回してしまっている                       
    for df_col in dp_index: 
        for df_index in dp_index:
            coordinate_list.append(coordinate_df.loc[df_index,df_col])      

    #背反関係の強さを最大和に合わせて正規化
    coordinate_sum_max = max(coordinate_list)
    for df_col in dp_index: 
        for df_index in dp_index:
            if coordinate_df.loc[df_index,df_col] != "-" and coordinate_df.loc[df_index,df_col] != "":
                coordinate_df.loc[df_index,df_col] = float(coordinate_df.loc[df_index,df_col])/coordinate_sum_max
